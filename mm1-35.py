...
def doLoop():
		...
		while (y <= max_y):
			...
			# x, y  - координаты текущей точки, проверяемой на наличие соседей (для возможности вычислить 2-ю производную)
				...
				if (
					...
					if x == x_own and y == y_own:			# @
						own_idx = xyt_dict[x_own, y_own]
						#print "own_idx = ", own_idx
						T[rowCount][own_idx] = 1
						F[rowCount] = t_own
						rowCount += 1
						x = round(x + h, hr)
						continue
	...
	temp_set_last = tStore[len(tStore) - 1]			# @

	def tempInpoint(x, y):							# @
		index = xyt_dict[x, y]						#
		#print "index ", index
		temp_in_point = temp_set_last[index]
		print temp_in_point

	print "left-bottom:"										# @
	tempInpoint(x_own - h, y_own - h)
	print "left:"
	tempInpoint(x_own - h, y_own)
	print "left-top:"
	tempInpoint(x_own - h, y_own + h)
	print "more left:"
	tempInpoint(x_own - h - h, y_own)
	print "own point:"
	print t_own
